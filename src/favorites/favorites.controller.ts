import { Controller, Delete, Get, HttpCode, HttpException, HttpStatus, Param, ParseUUIDPipe, Post } from '@nestjs/common';
...
export class FavoritesController {
  ...
  addTrackToFavorites(@Param('id', new ParseUUIDPipe()) id: string) {
    if (!this.favoritesService.isItExist(id, 'tracks')) {
      throw new HttpException(`Track with ID=${id} doesn't exist`, HttpStatus.UNPROCESSABLE_ENTITY);
    }
  ...
  addAlbumToFavorites(@Param('id', new ParseUUIDPipe()) id: string) {
    if (!this.favoritesService.isItExist(id, 'albums')) {
      throw new HttpException(`Album with ID=${id} doesn't exist`, HttpStatus.UNPROCESSABLE_ENTITY);
    }
  ...
  addArtistToFavorites(@Param('id', new ParseUUIDPipe()) id: string) {
    if (!this.favoritesService.isItExist(id, 'artists')) {
      throw new HttpException(`Artist with ID=${id} doesn't exist`, HttpStatus.UNPROCESSABLE_ENTITY);
    }
